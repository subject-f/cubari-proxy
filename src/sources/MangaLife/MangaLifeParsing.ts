...
export const parseMangaDetails = ($: CheerioStatic, mangaId: string): Manga => {
  ...
  tagSections[0].tags = entity.genre.map((elem: string) =>
    createTag({ id: elem, label: elem })
  );
  ...
  let desc = "";
  const hentai =
    entity.genre.includes("Hentai") || entity.genre.includes("Adult");
  ...
  for (const row of $("li", details).toArray()) {
    ...
    switch (text) {
      case "Type:": {
        ...
        tagSections[1].tags.push(
          createTag({ id: type.trim(), label: type.trim() })
        );
        break;
      }
      case "Status:": {
        status = $(row).text().includes("Ongoing")
          ? MangaStatus.ONGOING
...
export const parseChapters = ($: CheerioStatic, mangaId: string): Chapter[] => {
  const chapterJS: any[] = JSON.parse(
    $.root().html()?.match(regex["chapters"])?.[1] ?? ""
  ).reverse();
...
export const parseChapterDetails = (
  ...
  const variableName = data.match(
    /ng-src="https:\/\/{{([a-zA-Z0-9.]+)}}\/manga\/.+\.png/
  )?.[1];
  ...
  for (let i = 0; i < pageNum; i++) {
    ...
    pages.push(
      `https://${matchedPath}/manga/${mangaId}/${
        chapterInfo.Directory == "" ? "" : chapterInfo.Directory + "/"
      }${chapterImage}-${page}.png`
    );
  }
...
export const parseUpdatedManga = (
  { data }: any,
  time: Date,
  ids: string[]
): MangaUpdates => {
...
export const searchMetadata = (query: SearchRequest) => {
  ...
  const genre: string[] | undefined = query.includeGenre
    ? query.includeDemographic
      ? query.includeGenre.concat(query.includeDemographic)
      : query.includeGenre
    : query.includeDemographic;
  const genreNo: string[] | undefined = query.excludeGenre
    ? query.excludeDemographic
      ? query.excludeGenre.concat(query.excludeDemographic)
      : query.excludeGenre
    : query.excludeDemographic;
...
export const parseSearch = (data: any, metadata: any): PagedResults => {
  ...
  const directory: any[] = JSON.parse(
    data?.match(regex["directory"])?.[1] ?? ""
  )["Directory"];
  const imgSource =
    data?.match(regex["directory_image_host"])?.[1] ?? ML_IMAGE_DOMAIN;
  if (imgSource !== ML_IMAGE_DOMAIN) ML_IMAGE_DOMAIN = imgSource;

  for (const elem of directory) {
    let mKeyword: boolean =
      typeof metadata.keyword !== "undefined" ? false : true;
    let mAuthor: boolean = metadata.author !== "" ? false : true;
    let mStatus: boolean = metadata.status !== "" ? false : true;
    let mType: boolean =
      typeof metadata.type !== "undefined" && metadata.type.length > 0
        ? false
        : true;
    let mGenre: boolean =
      typeof metadata.genre !== "undefined" && metadata.genre.length > 0
        ? false
        : true;
    let mGenreNo: boolean =
      typeof metadata.genreNo !== "undefined" ? true : false;
    if (!mKeyword) {
      const allWords: string = [...(elem.al ?? []), elem.s ?? ""]
        .join("||")
        .toLowerCase();
    ...
    if (!mStatus) {
      if (
        (elem.st == "ongoing" && metadata.status == "ongoing") ||
        (elem.st != "ongoing" && metadata.ss != "ongoing")
      )
        mStatus = true;
    }
    ...
    if (!mGenre)
      mGenre = metadata.genre.every((i: string) => flatG.includes(i));
    if (mGenreNo)
      mGenreNo = metadata.genreNo.every((i: string) => flatG.includes(i));
...
export const parseTags = (data: any): TagSection[] => {
  ...
  const genres = JSON.parse(
    data.match(/"Genre"\s*: (.*)/)?.[1].replace(/'/g, '"')
  );
...
export const parseHomeSections = (
  ...
  const hotSection = createHomeSection({
    id: "hot_update",
    title: "HOT UPDATES",
    view_more: true,
  });
  ...
  const newTitlesSection = createHomeSection({
    id: "new_titles",
    title: "NEW TITLES",
    view_more: true,
  });
  ...
  const latest = JSON.parse(data.match(regex[latestSection.id])?.[1]).slice(
    0,
    15
  );
  const newTitles = JSON.parse(
    data.match(regex[newTitlesSection.id])?.[1]
  ).slice(0, 15);
  ...
  let imgSource =
    $(".ImageHolder")
      .html()
      ?.match(/ng-src="(.*)\//)?.[1] ?? ML_IMAGE_DOMAIN;
  if (imgSource !== ML_IMAGE_DOMAIN) ML_IMAGE_DOMAIN = imgSource;
    ...
    if (!mangaIds.has(id)) {
      ...
      manga.push(
        createMangaTile({
          ...
          title: createIconText({ text: title }),
          secondaryText:
            homepageSectionId !== "new_titles"
              ? createIconText({ text: time, icon: "clock.fill" })
              : undefined,
        })
      );
      mangaIds.add(id);
